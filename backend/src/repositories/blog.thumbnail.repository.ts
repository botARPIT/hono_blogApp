...
export const uploadFile = async (file: string, folderLocation: FolderLocation, userId: string) => {
    if (!file || !userId) throw new NotFoundError("User id or file name is missing", { message: "Kindly provide the file" })
    const fileName = `${userId}/${folderLocation}/${Date.now()}`;
    const upsertValue = folderLocation == "thumbnails" ? false : true
